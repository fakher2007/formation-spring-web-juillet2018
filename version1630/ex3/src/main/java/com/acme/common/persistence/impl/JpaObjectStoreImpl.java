...
public class JpaObjectStoreImpl implements ObjectStore {
	...
	public <T extends AbstractPersistentEntity<U>, U> Optional<T> getById(Class<T> clazz, U id, Function<T, ?>... fetchs) {
		...
		T entity = this.em.find(clazz, id);
		if(entity!=null) {
			for (Function<T, ?> function : fetchs) {
				Object lazyRelation = function.apply(entity);
	...
	@PostConstruct
	@ManagedOperation // => le bean (c'est à dire un proxy) sera exposé en JMX
	@Transactional(propagation=Propagation.SUPPORTS) // la propagation MANDATORY ne s'applique pas pour cette méthode
